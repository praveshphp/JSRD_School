var Zenix = function () {
  ...
  var handleCurrentActive = function () {
    for (var nk = location.protocol + '//' + location.host + location.pathname,
...
jQuery(document).ready(function () {
  $('#search-form').on('submit', function () {
    if($('#s').val()==''){
      var msg='<div class="alert alert-danger alert-dismissible fade show"><svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="me-2"><polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2"></polygon><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg><strong>Error!</strong> Please enter keyword for search.<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="btn-close"></button></div>';
      $('#jserror').html(msg);
      return false;
    }
  })
  $("#autosuggest-property").autocomplete({
    ...
    select: function (event, ui) {
      $('#autosuggest-property').val(ui.item.label);
      console.log(ui.item);
      return false;
    }
    ...
    let hash_id = "";
    if (item.ptype == 'Potential Property') {
      hash_id = "pp_" + item.id;
    } else if (item.ptype == 'Competitor Property') {
      hash_id = "cp_" + item.id;
    } else {
      hash_id = "pi_" + item.id;
    }
    return $("<li>")
      .append('<a href="' + path_dashboard + '#' + hash_id + '" class="dropdown-item" id="bs-select-5-1" tabindex="0" aria-setsize="3" aria-posinset="2"><span class="text">' + item.address + '(<b>' + item.ptype + '</b>)</span></a>')
      .appendTo(ul);
  };;
